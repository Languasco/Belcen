...
app.controller('ArqueoCajaController', function ($scope, RevisionPedidoServices, $location, ArqueoCajaServices, $timeout, auxiliarServices, GrupoDetServices ) {
        ...
        ArqueoCajaServices.get_Zonas_anexos(id_Anexos, $scope.id_usuario_Global)
            .then(function (res) {
                ...
                if (res.ok == true) {
                    ...
                    $scope.listados_changeCentroCosto_anexosFiltro(id_Anexos);
    ...
    $scope.listado_tipoEgresos = function () {
        $scope.loader = true;
        ArqueoCajaServices.get_tiposEgresos()
            .then(function (res) {
                $scope.loader = false;
                if (res.ok == true) {
                    $scope.tiposEgresos = [];
    ...
    $scope.mostrandoInformacion_cobranzasDevoluciones = function () {
        ...
        ArqueoCajaServices.get_informacionVentas_cobranzasDevoluciones($scope.Objeto_Parametro_arqueoCaja.id_Anexo, $scope.Objeto_Parametro_arqueoCaja.id_ZonaVta, $scope.Objeto_Parametro_arqueoCaja.id_CC, $scope.Objeto_Parametro_arqueoCaja.fechaArqueoCaja, $scope.id_usuario_Global)
            .then(function (res) {
                ...
                if (res.ok == true) {
                    ...
                    $scope.totalCobranzasDocumentos();
    ...
    $scope.totalCobranzasDocumentos = function () {
        let cobra = 0;
        for (item of $scope.cobranzas) {
            cobra += (!item.importe) ? 0 : item.importe;
        }
        $scope.totalCobranzas = cobra.toFixed(2);
    }

    $scope.totalDevolucionesDocumentos = function () {

        let devol = 0;         
        for (item of $scope.devoluciones) {
            devol += (!item.importe) ? 0 : item.importe;
        }
        $scope.totalDevoluciones = devol.toFixed(2);
    }
            ...
            if (unidadMonetaria == 'B') { // billete
                for (item of $scope.lista_billeteReparto) {
                    if (item.id_BilleteMoneda == obj.id_BilleteMoneda && item.tipo_BilleteMoneda == 'B') {
                        ...
                        item.importe = (Number(cant) * Number(valor));  

                        $scope.save_monedasBillete($scope.id_ArqueoCaja_Global, 1, item.id_BilleteMoneda, Number(cant), Number(valor), item.importe  )
            ...
            if (unidadMonetaria == 'B') { // billete
                for (itemB of $scope.lista_billeteCaja) {
                    ...
                    if (itemB.id_BilleteMoneda == obj.id_BilleteMoneda && itemB.tipo_BilleteMoneda == 'B') {
                        ...
                        itemB.importe = (Number(cant) * Number(valor));

                        $scope.save_monedasBillete($scope.id_ArqueoCaja_Global, 2, itemB.id_BilleteMoneda, Number(cant), Number(valor), itemB.importe)
                ...
                for (itemM of $scope.lista_monedasCaja) {
                    ...
                    if (itemM.id_BilleteMoneda == obj.id_BilleteMoneda) {
                        ...
                        itemM.importe = (Number(cant) * Number(valor));

                        $scope.save_monedasBillete($scope.id_ArqueoCaja_Global, 2, itemM.id_BilleteMoneda, Number(cant), Number(valor), itemM.importe)
    ...
    $scope.calcularTotales_general = function () {
        ...
        $scope.totalCajaChica = (totalBC + totalMC);
    }

    $scope.save_monedasBillete = function (id_ArqueoCaja, id_Tipo, id_BilleteMoneda, cantidad_Billete, valor_Billete , total_Billete) {

        //id_Tipo == 1 es reparto  ,  id_Tipo == 2 es Caja Chica 
        $scope.loaderSaveD = true;
        ArqueoCajaServices.save_billetesMonedasArqueo(id_ArqueoCaja, id_Tipo, id_BilleteMoneda, cantidad_Billete, valor_Billete, total_Billete , $scope.id_usuario_Global).then(function (res) {
            $scope.loaderSaveD = false;
            if (res.ok == true) {
    ...
    $scope.EdicionRegistros_egresos = function (obj) {
        ...
        $scope.Objeto_Parametro_egresos = {
            id_ArqueoCaja_Egresos: obj.id_ArqueoCaja_Egresos,
            id_ArqueoCaja: obj.id_ArqueoCaja,
            id_TipoEgreso: obj.id_TipoEgreso,
            fecha_Registro: obj.fecha_Registro,
            id_Anexo_Destino: obj.id_Anexo_Destino,
            id_CC_Destino: obj.id_CC_Destino,

            id_TipoDocumento: obj.id_TipoDocumento,
            ...
            id_TipoDocumento2: obj.id_TipoDocumento2,
        ...
        $timeout(function () {
            ...
            $('#cbo_tipoEgreso').val(obj.id_TipoEgreso).trigger('change.select2');
        ...
        ArqueoCajaServices.get_informacionVentas_Devoluciones($scope.Objeto_Parametro_arqueoCaja.id_Anexo, $scope.Objeto_Parametro_arqueoCaja.id_ZonaVta, $scope.Objeto_Parametro_arqueoCaja.id_CC, $scope.Objeto_Parametro_arqueoCaja.fechaArqueoCaja, $scope.id_usuario_Global)
            .then(function (res) {
                ...
                if (res.ok == true) {
                    ...
                    $scope.totalDevolucionesDocumentos();
                } else {
                    $scope.loaderSaveD = false;
                    auxiliarServices.NotificationMessage('Sistemas', 'Lo sentimos se produjo un error', 'error', '#ff6849', 2000);
                    alert(res.data);
    ...
    $scope.mostrandoInformacion_arqueosCaja = function () {
        ...
        ArqueoCajaServices.get_listados_arqueoCajaCab($scope.Objeto_Parametro_filtro.id_Anexo, $scope.Objeto_Parametro_filtro.id_ZonaVta, $scope.Objeto_Parametro_filtro.id_CC, fechaIni, fechaFin, $scope.Objeto_Parametro_filtro.id_estado, $scope.id_usuario_Global)
            .then(function (res) {
    ...
    $scope.EdicionRegistros_arqueoCajaCab = function (obj) {
       ...
       $timeout(function () {
           ...
           $scope.mostrar_arqueoCajaCab_billetesMonedas_edicion(obj.id_ArqueoCaja);
    ...
    $scope.mostrandoInformacion_arqueosCajaEdicion = function (id_ArqueoCaja) {

        $scope.id_ArqueoCaja_Global = id_ArqueoCaja;
        $scope.loaderSaveD = true;
        ArqueoCajaServices.get_arqueoCajaCab_edicion(id_ArqueoCaja, $scope.id_usuario_Global)
            .then(function (res) {
    ...
    $scope.mostrar_arqueoCajaCab_billetesMonedas_edicion = function (id_ArqueoCaja) { 
        $scope.loaderSaveD = true;
        ArqueoCajaServices.get_arqueoCaja_monedasBilletes_edicion(id_ArqueoCaja, $scope.id_usuario_Global)
            .then(function (res) {
