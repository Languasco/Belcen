...
app.controller('Pedidos_IIController', function ($scope, $location, $timeout, auxiliarServices, $q, PedidosServices, AuditarServices, EstadosServices, RevisionPedidoServices,Cliente_IIServices, GrupoDetServices, Documentos_MasivosServices, NotaCreditoDebitoServices ,VehiculoServices, AlmacenServices, TipoDocumentoServices, PersonalServices, PuntoVentaServices) {
    ...
    $scope.initAll = function () {
        ...
        auxiliarServices.changeTitle("Venta Directa New");
    ...
    $scope.change_Local_Almacen_pedido_new = function (idlocal) {        
        ...
        AlmacenServices.get_ListadoAlmacenes_Local_Usuario(idlocal, auxiliarServices.getUserId())
            .then(function (data) {
                ...
                setTimeout(function () {
                    if (data.length > 0) {
                        $scope.objeto_parametros.id_Almacen = String(data[0].id_Almacen);
                        $('#cbo_almacen_pedido').val(String(data[0].id_Almacen)).trigger('change.select2');
                        $scope.change_almacen_anexo_pedido(data[0].id_Almacen);
                    } else {
                        $scope.objeto_parametros.id_Almacen = '0';
                        $('#cbo_almacen_pedido').val("0").trigger('change.select2');
                    }                
    ...
    $scope.Buscar_Producto = async function () {
        ...
        $scope.loader_modal = true;
        PedidosServices.get_Search_Producto_pasaje(id_AlmacenPedido, $scope.objeto_parametros_detalle.codigo1_Producto, auxiliarServices.getUserId())
            .then(function (data) {
                ...
                const PrecVent = (!data[0].precioventa_listaprecios) ? '0' : data[0].precioventa_listaprecios;
               ...
               $scope.ListaUnidadMedidaModal = [];
               PedidosServices.get_unidadMedidasFactor(auxiliarServices.getUserId(), data[0].id_producto)
                   .then((res) => {
    ...
    $scope.change_unidadStock = function () {
        if ($scope.objeto_parametros_detalle.id_UnidadMedida_Venta == '0') {
            $scope.objeto_parametros_detalle.stock = 0;
            $scope.objeto_parametros_detalle.factorMultiplicacion_Venta = 0;
            $scope.objeto_parametros_detalle.precioVenta_Pedido_Det = 0;
        } else {
            ...
            const { id_unidadMedida, nombre_UnidadMedida, factor_UnidadMedida } = $scope.ListaUnidadMedidaModal.find((u) => u.id_unidadMedida == $scope.objeto_parametros_detalle.id_UnidadMedida_Venta);

            $scope.objeto_parametros_detalle.factorMultiplicacion_Venta = factor_UnidadMedida;
            if (idUnidad_Global != id_unidadMedida) {
                $scope.objeto_parametros_detalle.stock = (stock_Global * factor_Global / factor_UnidadMedida).toFixed(2)
                $scope.objeto_parametros_detalle.precioVenta_Pedido_Det = (precioVenta_Global / factor_Global * factor_UnidadMedida).toFixed(4)
            }
            else {
                $scope.objeto_parametros_detalle.stock = stock_Global;
                $scope.objeto_parametros_detalle.precioVenta_Pedido_Det = precioVenta_Global;
            }
        }
    }
    ...
    $scope.Guardar_Pedido_Det = function () {
        ...
        if ($scope.Flag_modoEdicion_detalle === false) { // nuevo registroo detalle
            ...
            //--Validando
            if (Existencia_Producto($scope.objeto_parametros_detalle.id_Producto, $scope.objeto_parametros_detalle.id_UnidadMedida_Venta  ) == true) {
               $timeout(function () {
                    $('#txt_cod_producto').focus().select();
                });
                return;
    ...
    function Existencia_Producto(id_producto, id_unidadMedida) {
        var result = false;

        console.log(id_unidadMedida)
        console.log($scope.Lista_DataDetalle)

        $scope.Lista_DataDetalle.forEach(function (item, index) {
            if (item.id_Producto === id_producto && item.id_UnidadMedida_Venta == id_unidadMedida ) {
                auxiliarServices.NotificationMessage('Sistemas', 'El Producto y la unidad Medida ya se encuentra registrado, verifique.!', 'error', '#ff6849', 2000);
                result = true;
            }
        });
    ...
    $scope.Buscar_Producto_Edicion = function (obj_detalle) {
        ...
        const PrecVent = (!obj_detalle.precioVenta_Pedido_Det) ? '0' : obj_detalle.precioVenta_Pedido_Det;
        ...
        //------ unidad de medida en linea -----
        $scope.ListaUnidadMedidaModal = [];
        PedidosServices.get_unidadMedidasFactor(auxiliarServices.getUserId(), obj_detalle.id_Producto)
            .then((res) => {
       ...
       //------Fin unidad de medida en linea -----
        ...
        $scope.loader_modal = true;
        PedidosServices.get_Search_Producto_pasaje($scope.objeto_parametros.id_Almacen, obj_detalle.codigo1_Producto, auxiliarServices.getUserId())
            .then(function (data) {
                ...
                let idUnidadStock = data[0].Id_Unidad 
                stock_Global = data[0].Stock;

                if (idUM != idUnidadStock) {
                    $scope.objeto_parametros_detalle.stock = (stock_Global * factor_Global)
                }
                else {
                    $scope.objeto_parametros_detalle.stock = stock_Global;
    ...
    $scope.Ayuda_BuscarProducto = function () { 
        ...
        $scope.loader_modal_ayuda = true;
        PedidosServices.get_Ayuda_Buscar_Producto_pasaje(id_AlmacenPedido, txt_busquedaProducto, auxiliarServices.getUserId())
            .then(function (data) {
                $scope.loader_modal_ayuda = false;
                $scope.Lista_Busqueda_Producto = [];
    ...
    $scope.Agregar_Producto = function (obj) {
        ...
        const PrecVent = (!obj.precioventa_listaprecios) ? '0' : obj.precioventa_listaprecios;
        ...
        //------ unidad de medida en linea -----
        $scope.ListaUnidadMedidaModal = [];
        PedidosServices.get_unidadMedidasFactor(auxiliarServices.getUserId(), obj.id_producto)
            .then((res) => {
    ...
    //--------  FILTROS DE COMBOS DE FACTURACION
