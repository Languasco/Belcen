var app = angular.module('appGestion.Pedidos_DirectoController', [])
app.controller('CtrlPedidos_Directo', function ($scope, loginServices, $location, $timeout, $q, auxiliarServices, Cliente_IIServices,  PedidosServices, LocalesServices, GrupoDetServices, Documentos_MasivosServices,NotaCreditoDebitoServices, VehiculoServices, AlmacenServices, TipoDocumentoServices, PersonalServices, PuntoVentaServices, CondicionFacturacionServices) {
    ...
    $scope.initAll = function () {
        ...
        auxiliarServices.changeTitle("Pedidos Directo");
        $scope.titleModal = "Generación de Pedidos Directo";
    ...
    $scope.change_Local_Almacen_pedido = function (idlocal) {
        ...
        TipoDocumentoServices.getTipoDocumento(0)
            .then(function (data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    else if (data[i].id_TipoDocumento === 4 || data[i].id_TipoDocumento === 5) {
                        $scope.Lista_TipoGuiaRemision.push(data[i]);
                    }
                }
    ...
    $scope.clean = function () {
        ...
        $timeout(function () {
            ...
            if ($scope.Lista_Local !== null || $scope.Lista_Local !== undefined || $scope.Lista_Local !== '' || $scope.Lista_Local !== []) {
                $scope.objeto_parametros.id_Local = String($scope.Lista_Local[0].id_Local);
                $('#cbo_local_pedido').val(String($scope.Lista_Local[0].id_Local)).trigger('change.select2');
                $scope.change_Local_Almacen_pedido($scope.objeto_parametros.id_Local);
            } else {
                $scope.objeto_parametros.id_Almacen = '0';
                $scope.objeto_parametros.id_Local = '0';
                ...
                PedidosServices.save_Pedido($scope.objeto_parametros)
                    .then(function (data) {
                        ...
                        $timeout(function () {
                            ...
                            auxiliarServices.initSweetAlert(error).then(function (res) {
    ...
    $scope.EdicionRegistros = function (obj) {
        ...
        $scope.change_Local_Almacen_pedido(obj.id_Local);
        ...
        $timeout(function () {
            ...
            $('#cboCanales').val(obj.cboCanales).trigger('change.select2');
    ...
    $scope.GuardarCliente = function () {
        ...
        //if ($scope.objeto_parametros_cliente.id_DocumentoIdentidad == 21 || $scope.objeto_parametros_cliente.id_DocumentoIdentidad == '21') {  // ruc
        //    if (VerificaRuc($scope.objeto_parametros_cliente.nroDoc_Cliente) == false) {
        //        auxiliarServices.NotificationMessage('Sistemas', 'El nro de R.u.c ingresado es incorrecto, verifique..', 'error', '#ff6849', 2500);
        //        return;
        //    }
                    ...
                    PedidosServices.set_rechazar_Pedido(obj.Numero_Pedido, auxiliarServices.getUserId())
                        .then(function (data) {
                            ...
                            if (data === "OK") {
                                ...
                                $scope.Lista_DataCabecera[index].estado = 27;
    ...
    $scope.change_tipoDoc = function (obj) {
        if (obj === 3) {
            $scope.checked = "ng - model='checked'";
        } else {
            $scope.checked = "";
        }
    };
                ...
                res.forEach(function (item, index) {
                    if (idCab !== item.IDCAB) {
                        ...
                        cabContent.push({
                            ...
                            DISTRITO_EMITE: item.DISTRITO_EMITE,
                            CALLE_EMITE: item.CALLE_EMITE,
                            FLAG_EXONERA: item.FLAG_EXONERA,
                            DETCONTENT: []
                        });
                        detConent.forEach(function (itemdet) {
                            if (itemdet.IDCAB === cabContent[indexAux].IDCAB) {
    ...
    $scope.GenerarFacturacion_new = function () {
        var indicador = false;
        for (var i = 0; i < $scope.Lista_DataDetalle.length; i++) {
            indicador = true;
            break;
        ...
        auxiliarServices.initSweetAlert(params).then(function (res) {
            if (res == true) {
                $scope.loader_modal = true;
                $('#btn_generaCancelacion').attr("disabled", true);
                PedidosServices.set_GenerandoFacturacion($scope.objeto_parametros.Numero_Pedido, auxiliarServices.getUserId(), $scope.objeto_parametros.flag_tipo_facturacion)
                    .then(function (res) {
                        $scope.loader_modal = false;
    ...
    $scope.change_exonerada = function () {
        var chk_exonerada = document.getElementById('chk_exonerada');
        if ($scope.objeto_parametros.Numero_Pedido == '' || $scope.objeto_parametros.Numero_Pedido == '0') {
    ...
    $scope.Actualizar_flag_Exonerada = function () {
        ...
        $scope.loader_modal = true;
        PedidosServices.Actualizar_flag_Exonerada($scope.objeto_parametros.Numero_Pedido, exonerada)
            .then(function (ok) {
                $scope.loader_modal = false;
    ...
    $scope.cambiar_Facturacion = function (tipo) {
        ...
        if ($scope.objeto_parametros.Numero_Pedido == '' || $scope.objeto_parametros.Numero_Pedido == '0') {
            ...
            $scope.objeto_parametros.flag_tipo_facturacion = '1';
            $timeout(function () {
                $('#cbo_tipo_Factura').val("10").trigger('change.select2');
            }, 200);
        } else {
    ...
    $scope.Actualizar_flag_Tipo_facturacion = function (tipoFactura) {
        $scope.loader_modal = true;
        PedidosServices.Actualizar_flag_tipo_facturacion($scope.objeto_parametros.Numero_Pedido, tipoFactura)
            .then(function (ok) {
                $scope.loader_modal = false;
                $scope.CalculoTotales_General_new();
            }, function (error) {
