...
app.controller('DocumentosManualesController', function ($scope, $location, $timeout, auxiliarServices, $q, PedidosServices, AuditarServices, EstadosServices, RevisionPedidoServices, Cliente_IIServices, GrupoDetServices, Documentos_MasivosServices, NotaCreditoDebitoServices, VehiculoServices, AlmacenServices, TipoDocumentoServices, PersonalServices, PuntoVentaServices, DocumentosManualesServices) {
    ...
    $scope.initAll = function () {
        ...
        auxiliarServices.changeTitle("Generar Documento Manual");
        $scope.titleModal = "Generar Documento Manual";
    ...
    $scope.Listando_InformacionPedidos = function () {
        ...
        $scope.loaderfiltros = true;
        DocumentosManualesServices.get_PedidosManual($scope.Objeto_ParametroFiltro)
            .then(function (data) {
                $scope.Lista_DataCabecera = [];
    ...
    $scope.Guardar_Pedido_Cab = function () {
        ...
        if ($scope.objeto_parametros.serie == 0 || $scope.objeto_parametros.serie == '0' || $scope.objeto_parametros.serie == null || $scope.objeto_parametros.serie == '') {
            auxiliarServices.NotificationMessage('Sistemas', 'Por favor ingrese la serie', 'error', '#ff6849', 1500);
            return false;
        }
        if ($scope.objeto_parametros.num_doc == 0 || $scope.objeto_parametros.num_doc == '0' || $scope.objeto_parametros.num_doc == null || $scope.objeto_parametros.num_doc == '') {
            auxiliarServices.NotificationMessage('Sistemas', 'Por favor ingrese la serie', 'error', '#ff6849', 1500);
            return false;
        }
                ...
                $scope.loader_modal = true;
                 DocumentosManualesServices.verificacionNumeroCorrelativo($scope.objeto_parametros.id_Anexos, $scope.objeto_parametros.id_TipoDocumento, $scope.objeto_parametros.Numero_Documento)
                    .then(function (res) {
                        ...
                        if (res.ok == true) {
                            const cantReg = res.data[0].cantRegistro;
                            if (cantReg == 0) {
                                ////-----guardando el pedido----
    ...
    $scope.Buscar_Producto = function () {
        ...
        $scope.loader_modal = true;
        PedidosServices.get_Search_Producto_manual(id_AlmacenPedido, $scope.objeto_parametros_detalle.codigo1_Producto, auxiliarServices.getUserId())
            .then(function (data) {
    ...
    $scope.Ayuda_BuscarProducto = function () { 
        ...
        $scope.loader_modal_ayuda = true;
        PedidosServices.get_Ayuda_Buscar_Producto_Manual(id_AlmacenPedido, txt_busquedaProducto, auxiliarServices.getUserId())
            .then(function (data) {
                $scope.loader_modal_ayuda = false;
                $scope.Lista_Busqueda_Producto = [];
    ...
    $scope.GenerarFacturacion = function () {
        ...
        auxiliarServices.initSweetAlert(params).then(function (res) {
            if (res == true) {
                ...
                DocumentosManualesServices.set_GenerandoFacturacionManual($scope.objeto_parametros.id_Pedido_Cab, auxiliarServices.getUserId(), $scope.objeto_parametros.flag_tipo_facturacion)
                    .then(function (res) {
                        $scope.loader_modal = false;                     
                        if (res.ok == true) {
                            $('#btn_generaCancelacion').attr("disabled", false);
            ...
            if (res === true) {
                ...
                PedidosServices.set_Generando_Pagos($scope.objeto_parametros_pagos)
                    .then(function (data) {
                        ...
                        console.log('set_Generando_Pagos')
                        console.log(data)

                        if (data === "OK") {

                            ///----almacenando la imagen en el servidor
                            $scope.upload_imageComprobante($scope.id_Pedido_Cab_Global, 0);
...
/*        if (auxiliarServices.getAnularDoc() === 1) {*/
    ...
    $scope.upload_imageComprobante = function (id_Pedido_Cab, idFacturaCab) {

        $scope.loader_modal_ayuda = true;
        PedidosServices.uploadFile_imageComprobante($scope.files[0].file, auxiliarServices.getUserId(), id_Pedido_Cab, idFacturaCab)
            .then(function (res) {
                console.log('uploadFile_imageComprobante');
                console.log(res);
