...
namespace Negocio.StockAlmacen
{
    public class GetStockAlmacen_BL
    {
        ...
        public object getKardexAlmacen_new(string fechaini, string fechafin, int id_material, int tipo, int local, int almacen)
        {
            ...
            using (SqlConnection cnDB = new SqlConnection(bdConexion.cadenaBDcx()))
            {
                ...
                while (dr.Read())
                {
                    ...
                    objStockKardex.PrecioUnitario = dr["PrecioUnitario"].ToString();
                    objStockKardex.Almacen = dr["Almacen"].ToString();
                    objStockKardex.Responsable = dr["Responsable"].ToString();
                    objStockKardex.Estado = dr["Estado"].ToString();
                    objStockKardex.UsuarioCreacion = dr["UsuarioCreacion"].ToString();
        ...
        public object getKardexValorizado_Todo(string fechaini, string fechafin, int tipo, int local, int almacen)
        {
            listKardexAlmacen = new List<kardesAlmacen>();
            string result = "";
            using (SqlConnection cnDB = new SqlConnection(bdConexion.cadenaBDcx()))
            {
                cnDB.Open();
                SqlCommand cmd = new SqlCommand("SP_GET_KARDEX_VALORIZADO_TODO", cnDB);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add("@FECHAINI", SqlDbType.VarChar).Value = fechaini;
                cmd.Parameters.Add("@FECHAFIN", SqlDbType.VarChar).Value = fechafin;
                cmd.Parameters.Add("@id_local", SqlDbType.Int).Value = local;
                ...
                result = generarExcel_KardexValorizado_Todo(listKardexAlmacen, fechaini);
            }
            return result;
        }

        public string generarExcel_KardexValorizado_Todo(List<kardesAlmacen> listKardexAlmacen, string fechaini)
        {
            string Res = "";
            string _servidor;
            string _ruta;
            ...
            {
                _servidor = String.Format("{0:ddMMyyyy_hhmmss}.xlsx", DateTime.Now);
                FileRuta = System.Web.Hosting.HostingEnvironment.MapPath("~/ArchivosExcel/" + "kardexValorizadoTodo_" + _servidor);
                string rutaServer = ConfigurationManager.AppSettings["servidor_archivos"];

                FileExcel = rutaServer + "kardexValorizadoTodo_" + _servidor;
                FileInfo _file = new FileInfo(FileRuta);
                if (_file.Exists)
                {
                    _file.Delete();
                ...
                using (Excel.ExcelPackage oEx = new Excel.ExcelPackage(_file))
                {
                    Excel.ExcelWorksheet oWs = oEx.Workbook.Worksheets.Add("kardexValorizado");


                    oWs.Cells[1, 1].Value = "REPORTE KARDEX VALORIZADO TODOS: ";
                    oWs.Cells[1, 1].Style.HorizontalAlignment = Style.ExcelHorizontalAlignment.Center; // alinear texto  
                    oWs.Cells[1, 1, 1, 18].Merge = true;  // combinar celdaS 
                    oWs.Cells[1, 1].Style.Font.Size = 15; //letra tamaño   
