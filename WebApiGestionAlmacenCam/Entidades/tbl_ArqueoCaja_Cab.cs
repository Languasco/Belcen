//------------------------------------------------------------------------------
// <auto-generated>
//     Este código se generó a partir de una plantilla.
//
//     Los cambios manuales en este archivo pueden causar un comportamiento inesperado de la aplicación.
//     Los cambios manuales en este archivo se sobrescribirán si se regenera el código.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Entidades
{
    using System;
    using System.Collections.Generic;
    
    public partial class tbl_ArqueoCaja_Cab
    {
        public int id_ArqueoCaja { get; set; }
        public Nullable<int> id_ZonaVta { get; set; }
        public Nullable<int> id_Anexo { get; set; }
        public Nullable<int> id_CC { get; set; }
        public Nullable<System.DateTime> fechaArqueoCaja { get; set; }
        public string nroArqueoCaja { get; set; }
        public Nullable<int> id_Personal_Rinde { get; set; }
        public Nullable<int> id_Personal_Responsable { get; set; }
        public Nullable<int> id_Personal_Supervisor { get; set; }
        public Nullable<System.DateTime> fechaSaldoInicial { get; set; }
        public Nullable<decimal> importeSaldoInicial { get; set; }
        public Nullable<int> estado { get; set; }
        public Nullable<int> usuario_creacion { get; set; }
        public Nullable<System.DateTime> fecha_creacion { get; set; }
        public Nullable<int> usuario_edicion { get; set; }
        public Nullable<System.DateTime> fecha_edicion { get; set; }
    }
}
